...
export default function PlantPullExperience({ bannerSlug, costGems, dailyFree, dailyPullAvailable = true }: { bannerSlug: string; costGems: number; dailyFree: boolean; dailyPullAvailable?: boolean }) {
  ...
  const [stage, setStage] = useState<"idle" | "planting" | "revealed">("idle");
  const [results, setResults] = useState<Character[]>([]);
  const [selectedCount, setSelectedCount] = useState<1 | 10>(1);
  const [error, setError] = useState<string | null>(null);

  async function pull() {
    if (dailyFree && !dailyPullAvailable) return;
    setStage("planting"); setResults([]); setError(null);
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, selectedCount === 10 ? 1800 : 1300));
      setResults(result.characters); setStage("revealed");
      if (!dailyFree) {
  ...
  return (
    <div className="mt-2">
      <div className={`relative mx-auto flex min-h-[360px] w-full max-w-4xl items-center justify-center overflow-hidden rounded-3xl border border-lime-300/30 bg-[radial-gradient(ellipse_at_center,_rgba(132,204,22,0.28),_rgba(16,35,24,0.8)_48%,_rgba(2,6,23,1)_100%)] p-6 shadow-2xl shadow-lime-950/30 sm:min-h-[440px] sm:p-10 ${stage === "planting" ? "animate-pulse" : ""}`}>
        ...
        </div> : <div className="relative text-center"><div className={`text-8xl ${stage === "planting" ? "animate-bounce" : "animate-[spin_8s_linear_infinite]"}`}>{stage === "planting" ? "🌱" : "💎"}</div><div className="mt-6 flex justify-center gap-2 text-xl text-lime-300"><span className="animate-bounce">✦</span><span className="animate-bounce [animation-delay:150ms]">✦</span><span className="animate-bounce [animation-delay:300ms]">✦</span></div><p className="mt-4 text-sm text-slate-300">{stage === "planting" ? "The garden is waking up…" : "Plant a gem and see what grows."}</p></div>}
      </div>
      <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
        {!dailyFree && <div className="flex rounded-xl border border-slate-700 bg-slate-900 p-1"><button type="button" onClick={() => setSelectedCount(1)} disabled={stage === "planting"} className={`rounded-lg px-4 py-2 text-sm font-semibold ${selectedCount === 1 ? "bg-lime-300 text-garden-950" : "text-slate-300"}`}>1 pull · 💎 {costGems}</button><button type="button" onClick={() => setSelectedCount(10)} disabled={stage === "planting"} className={`rounded-lg px-4 py-2 text-sm font-semibold ${selectedCount === 10 ? "bg-lime-300 text-garden-950" : "text-slate-300"}`}>10 pulls · 💎 {costGems * 10}</button></div>}
        <button type="button" onClick={pull} disabled={stage === "planting" || (dailyFree && !dailyPullAvailable)} className="rounded-xl bg-brand px-6 py-3 text-sm font-bold text-garden-950 transition hover:bg-lime-300 disabled:cursor-not-allowed disabled:opacity-50">{stage === "planting" ? "Growing…" : results.length ? "Plant again" : dailyFree && !dailyPullAvailable ? "Come back tomorrow" : dailyFree ? "Plant today’s free pull" : `Plant ${selectedCount === 10 ? "10 pulls" : "a pull"}`}</button>
      </div>
